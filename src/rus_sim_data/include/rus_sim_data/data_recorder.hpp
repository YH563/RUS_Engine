...
namespace RusSimData {
    ...
    public:
         ...
         * @brief 录制一帧状态
         */
        void RecordFrame(const RusUtils::RobotState& state);
         ...
         * @return true 索引有效
         */
        bool GetFrame(size_t index, RusUtils::RobotState& state) const;
    ...
    private:
        ...
        /** 将一帧写入已打开的文件 */
        void write_frame(const RusUtils::RobotState& state);
        ...
        std::ofstream file_;
        std::vector<RusUtils::RobotState> frames_;  /**< 加载到内存，用于回放 */

        // ── 写线程 ──
        std::thread writer_thread_;
        std::atomic<bool> stop_writer_{false};
        std::deque<RusUtils::RobotState> write_queue_;
